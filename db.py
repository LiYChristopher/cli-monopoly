...
class DbInterface(object):
	...
	def card_info(self, cursor, card_type, card_name):
		...
		cursor.execute("SELECT tag, type, category, name, description, effect\
						FROM cards WHERE type = %s AND name=%s", (card_type, card_name,))
		result = cursor.fetchone()
	...
	def prop_set_length(self, cursor, card_type):
		card_type = card_type.lower()
		cursor.execute("SELECT COUNT(name) FROM property WHERE type=%s AND NOT type='utility'\
											AND NOT type='rr'", card_type)
		result = cursor.fetchone()
		return result
	...
	def rent_table(self, cursor):
		...
		return {prop[0]: {'type': prop[1], 'rent': prop[2] } for prop in result}
